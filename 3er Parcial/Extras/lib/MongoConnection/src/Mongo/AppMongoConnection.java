...
public class AppMongoConnection {
	...
	public void InsertarTabla() throws IOException
	{
		...
		File file=new File("C:\\JuegoSnake\\DB.txt");
		...
		while((linea=br.readLine())!=null)
		{
			...
			Puntaje=Integer.parseInt(st.nextToken().trim());
		}
		br.close();
		try
		{
			Mongo mongo=new Mongo("localhost",27017);
			Obj.db=mongo.getDB("SnakePuntajes");
		...
		documento.put("Puntaje", Puntaje);
		Obj.tabla.insert(documento);
	}
	
	public void DesplegarDB()throws IOException
	{
		AppMongoConnection Obj = new AppMongoConnection();
		File file=new File("C:\\JuegoSnake\\DBConsole.txt");
		FileWriter fw;
		BufferedWriter bw;
